...
Anthropic calls this **multi-turn propagation**. If the agent makes a 5% error in step 2—maybe it misinterprets a column name in a SQL database—that error doesn't just stay at 5%. It compounds. By step 10, the agent isn't just slightly off; it's hallucinating an entirely different reality. This makes traditional "static" evals (where you just check the final output string) almost useless for diagnosing *why* an agent failed.
...
Anthropic proposes two diverging metrics:

#### 1. pass@k (The "Best Case" Metric)
This is the probability that the agent succeeds **at least once** in *k* attempts. 
*   **Intuition:** "Can the agent solve this if it gets lucky?"
*   **Use Case:** Coding assistants or human-in-the-loop tools. If I’m using an agent to write a complex Python script and I can generate 5 versions, I only need **one** of them to be correct. 
*   **The Trend:** As you increase *k*, the score goes **UP**.

#### 2. pass^k (The "Reliability" Metric)
This is the probability that the agent succeeds **every single time** in *k* attempts.
*   **Intuition:** "Is this agent reliable enough to be left alone?"
*   **Use Case:** Fully autonomous agents (e.g., a customer support agent or a devops agent). If I run it 10 times and it fails once, that’s a 10% failure rate that could hit a real user.
*   **The Trend:** As you increase *k*, the score goes **DOWN** very fast.

| Metric | Goal | Statistical Meaning |
| :--- | :--- | :--- |
| **pass@k** | Capability | Measuring the ceiling of what is possible. |
| **pass^k** | Reliability | Measuring the floor of what is guaranteed. |

Anthropic’s point is that for a "production-ready" agent, you should be optimizing for a high **pass^k** (where $k \ge 5$). It's a much higher bar, but it's the only one that matters for autonomy.
...
3.  **Human Calibration:** This is the part everyone skips. You have to have a human grade a subset of the tasks, then compare the LLM's scores. If they don't match, you refine the judge's prompt. 
...
When the holes in the cheese don't align, the failure is caught.
